...
carthage_xcodeproj_path="Carthage Project/AnimatableTableView.xcodeproj"
carthage_pbxproj_path="${carthage_xcodeproj_path}/project.pbxproj"
swift_files=$(find 'AnimatableTableView/Classes' -type f -name "*.swift" | grep -o "[0-9a-zA-Z+ ]*.swift" | sort -fu)
swift_files_count=$(echo "${swift_files}" | wc -l | tr -d ' ')

build_section_id=$(sed -n -e '/\/\* AnimatableTableView \*\/ = {/,/};/p' "${carthage_pbxproj_path}" | sed -n '/PBXNativeTarget/,/Sources/p' | tail -1 | tr -d "\t" | cut -d ' ' -f 1)
...
set -o pipefail && xcodebuild -workspace "Pods Project/AnimatableTableView.xcworkspace" -scheme "AnimatableTableView-Example" -configuration "Release" -sdk iphonesimulator | xcpretty
